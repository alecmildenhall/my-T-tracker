...
- **Ask the real question, not a cheaper one that resembles it.** When you need to know whether something will work, **do it and check the result** — never run a proxy and infer. It has cost two defect *classes* so far — the second recurring three times across four review rounds — and every time the proxy differed from the real thing in a dimension nobody had thought about:
  ...
  **A test can be the proxy, and that is the dangerous form** — the two cases above are production code predicting instead of doing, but a guard that checks a stand-in passes while the thing it protects is broken, and its green is then read as evidence. Three CSS-cascade defects on the pain-chips branch were "pinned" by a test asserting **source order**, which is a stand-in for the question *which declaration wins*: a pseudo-class adds specificity, so `.button:hover` (0,2,0) beats a single-class confirmed state wherever either sits. The order test passed while the Done button showed "✓ Done" on hover-blue for its entire beat — and since the cursor is by definition still on a button you just clicked, the green confirmation was never visible on desktop at all. The replacement asserts **which colour wins** for an element in a given state, and it catches the first fix, which was also wrong (two rules set that background, not one). So: a guard must assert the **outcome the rule exists to protect**, never an arrangement that usually produces it. Guards that pass vacuously have now been found in the reduced-motion check, the focus-ring check and this one, each after unrelated code moved around them — so **mutation-test every new assertion, and confirm the mutation actually applied.**
...
- **Optional numbers are checked with `typeof x === "number"`, never truthiness** — `0` is a valid `doseMg` (and a valid days-late), so `x && …` silently drops it. See `daysFromPlanned` in `schedule.ts`, and `pickShotFields` in `backupDto.ts`, which keeps `doseMg` on `!== undefined` for exactly this reason. _(This rule used to cite `painScore`, which became an enum — the rule outlived its example, and `pain` now takes a real type guard instead.)_
...
- **Form controls never render below 16px on a touch screen.** iOS Safari zooms the whole page when you focus a control whose text is smaller — not a setting, not configurable, just what the browser does — and at `0.85rem` (13.6px) tapping any field jumped the layout on the app's primary platform, in its primary flow. One rule sets it (`@media (any-pointer: coarse)` in `styles.css`); **do not add a `font-size` to any input, textarea or select elsewhere**, because a class selector out-specifies that rule and a media query adds no specificity — that is exactly how the rename dialog kept zooming after the rest of the app stopped. `any-pointer`, not `pointer`: an iPad with a Magic Keyboard reports `pointer: fine` while you are still tapping the screen. And never "fix" this with `user-scalable=no` or `maximum-scale` — it disables pinch-zoom for everyone, fails WCAG 1.4.4, and iOS ignores it anyway. A test enumerates every rule that sizes a form control and pins the list.
