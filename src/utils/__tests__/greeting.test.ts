...
const OTHER_WEEKDAY = ORDINARY_WEEKDAY === "monday" ? "tuesday" : "monday";
...
describe("resolveGreeting — milestone (top priority)", () => {
  it("celebrates with the name", () => {
    expect(
      resolveGreeting(
        profile({ startDate: START, preferredName: "Lou" }),
        true,
        ONE_YEAR_DAY,
      ),
...
describe("resolveGreeting — first-time (no shots, no active milestone)", () => {
  ...
  it("still welcomes when a start date is set but no milestone is active", () => {
    expect(
      resolveGreeting(profile({ startDate: START }), false, ORDINARY_DAY),
    ).toBe("Welcome :)");
  });
});
...
describe("resolveGreeting — name handling", () => {
  it("treats a blank name as absent", () => {
    expect(
      resolveGreeting(profile({ preferredName: "" }), true, ORDINARY_DAY),
    ).toBe("Hi there~");
  });
  ...
  it("keeps a name with internal spaces", () => {
    expect(
      resolveGreeting(
        profile({ preferredName: "Lou Smith" }),
        true,
        ORDINARY_DAY,
      ),
    ).toBe("Hi, Lou Smith~");
...
describe("resolveGreeting — shot day against a non-weekly interval", () => {
  it("goes quiet while the interval is not a whole number of weeks", () => {
    // The control is greyed out in Settings for this case, because a weekday
    // cannot describe a 10-day rhythm — the grid would walk across the week. A
    // disabled setting that still fired a greeting every Wednesday would be the
    ...
    ).not.toMatch(/shot day/i);
  });

  it("comes back when the interval returns to a whole number of weeks", () => {
    // Read through, not cleared: correcting a mistyped interval gets the day
  ...
  it("still greets when no interval is set at all", () => {
    // Shot day predates cadence and keeps its original job for anyone who
    // never sets one.
    expect(
      resolveGreeting(
        profile({ shotDays: [ORDINARY_WEEKDAY] }),
