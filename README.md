...
- **Turn-off-able.** On by default, because the point is that it should be there — but being affirmed by software is a legitimate thing to dislike, and one neutral switch in Settings covers it. The switch **must be added to `pickProfileFields`** (the backup DTO allowlist) or it silently will not survive export and restore: `replaceProfile` swaps the whole profile on import, so the person who deliberately turned this off would get it back on after restoring their own backup. Not data loss, and one tap to fix — but this is the one preference in the app whose whole purpose is opting out of being affirmed, so quietly overriding it is worse than the mechanical cost suggests. The allowlist already carries the other preferences (`shotDay`, `preferredName`), so leaving this one out would be an inconsistency rather than a principled line.
