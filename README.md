...
- [x] _Engineering:_ **the Modal's Tab trap wants its own owner** — _done: `useFocusTrap` (`src/hooks/useFocusTrap.ts`) owns Tab and Escape, `tabbablesIn` (`src/utils/tabbing.ts`) wraps `tabbable`._ The focus-hand-off PR consolidated eight `.focus()` sites into `handOffFocus`, and in doing so rewrote the trap twice — each rewrite fixing the previous one's escape and introducing the next defect. Four review rounds on that one file: the wrap target ignored disabled controls; then edge detection ignored them too; then owning every Tab conflated "inside on a `tabIndex={-1}` element" with "outside the dialog"; then owning every Tab cancelled `input[type=date]`'s own segment stepping. Every one was invisible to jsdom and found either in a browser or by review.
  ...
  **What is still open**, deliberately — and the first item is a correction, because an earlier draft of this note claimed more than the work delivered.

  **Stacking is fixed for the keyboard and for focus, and NOT for modality.** The registry makes Tab, Escape and the focus net obey the topmost dialog. Everything else `Modal` does on open is still per-instance and unconditional, so a *second* dialog does not compose:
    - **`inert` is dropped too early.** Each Modal sets `inert` on `#root` at open and removes it at close, with no refcount — so closing an inner confirm lifts it while the outer sheet is still open, leaving the tab bar and the shot list clickable and screen-reader-reachable underneath. Measured: `inert` is present with the outer open, present with both open, and **gone once the inner closes**. `--sheet-h` has the same shape, so an outer sheet also loses its keyboard-aware height. (`document.body.style.overflow` is fine by accident — it captures and restores the previous value rather than a fixed one.)
    - **`inert` is only ever applied to `#root`**, never to the dialog underneath, so a confirm opened over the sheet leaves the sheet's own Save button clickable behind it.
    - **Back is worse than "does nothing".** Each Modal pushes its own overlay entry and registers its own unguarded `popstate` handler. The first Back lands on the outer's overlay entry and both handlers return early — but the second lands on a non-overlay entry and fires `onClose` on **both**. Measured: after two pops, each `onClose` has been called once. In B½ terms that is a delete-confirm over a half-filled log sheet, where two Back presses dismiss the confirm *and* discard the sheet, with no undo.

  So **a confirm opened from inside a sheet is not supported yet** — the focus half of it is, which is the part that was blocking. The remaining three are one small piece of work (the registry makes refcounting cheap) and should be done together, deliberately, rather than folded into a PR about the trap: `useBackToClose` is StrictMode-sensitive and sits in the most defect-dense area of the codebase, and fixing focus, Escape, Back and modality at once is how slice B became a nine-defect epic.
...
- [ ] **History & Charts epic** (local-first, phone-first) — progressive-disclosure information architecture: Home shows a *recent-shots teaser* + "See all"; a dedicated **History** tab holds the full, filterable, searchable list **and** the charts; charts summarize trends. The unbounded list is never the main view. Built as thin slices, in order, after the CivilDate PR:
  ...
  **What we actually track (the signals the charts are built on).** Deliberately *not* pain-as-headline: pain is the easiest thing to log and the least interesting thing to chart. Industry pattern across the apps worth emulating (Daylio, Clue, Medisafe, Apple Health) is **fast ordinal scales + categorical tags**, with correlation — not raw numbers — as the payoff.
  ...
  - **Adherence & lateness** — the objective early hero (meaningful by ~shot 3, while trend charts still say "not enough data yet"). Needs a lightweight **interval/cadence** concept. This bullet used to specify `expected = previous shot's date + interval, defaulted from the user's most common gap`, and **both halves of that were wrong** — the implementation rejects them by name:
    ...
    **Refinement, decided at the same time: picking more than one day implies weekly, and the interval question goes away.** *"If it's multiple times a week then we should skip interval since it really should be weekly at that point — I don't think people take it multiple times a week who don't do it every week."* That is right, and it removes a question the first sketch would have asked and that has no good answer: having chosen Monday and Thursday, being asked "and how often?" invites "twice a week", which is not what the field means. So:

    - **Two or more days selected ⇒ the cadence is weekly, and `intervalDays` is written as 7 and its control disabled** — not left blank and implied. An implied value is a second meaning for an empty field, which is the bug class this codebase opens its style guide with. Writing it keeps stored state and behaviour identical, and deselecting back to one day leaves a 7 on screen that can be edited.
    - **This mirrors a control the app already has.** Shot day disables itself when the interval is not a whole number of weeks, with a notice above it saying why. Interval disabling itself when several days are chosen is the same shape pointing the other way, so the pattern is learned once.
    - **Multiple days on a non-weekly cycle gets nothing, deliberately** — Mon/Thu every *other* week is real but rare, and the answer is the one the app already gives to every cadence it cannot describe honestly: leave the fields empty and get no planned dates. Consistency with the 10-day case matters more than covering it.
    - **The control becomes a row of day toggles, not a multi-select.** Seven small chips are the idiomatic phone pattern, they read at a glance as a *pattern* rather than a list, and the app's chip vocabulary already exists. A `<select multiple>` on a phone is close to unusable.
  ...
  ***
  ...
  - [ ] **B½ — Logging model** (data model + log form; no charts). Lands *between* B and D because the roadmap's own rule is to stabilize the data model before building charts on it. Covers the **interval/cadence** concept + **frozen lateness** fields stored on the shot, the **None/Mild/Moderate/Severe pain chips**, the **"Any days you felt off?"** question, **symptom tags**, **how the site settled** (retrospective soreness duration + unabsorbed lump), and the **opt-in "Bleeding" category** with the full safety model above. Absorbs three previously mid-term items (improved mood encoding, symptom tagging, friendlier pain scale). Import/export schema and the DTO allowlist grow with it; all fields stay optional and PII-free.
    ...
    **Ships as several PRs, not one.** Six features in one slice is what turned slice B into a nine-defect epic, and this one touches the same sheet. The order, and what gates what:
    ...
    1. ~~**The Modal's Tab trap**~~ — **done** (PR #34). It went first because it was the only thing here that *blocked*: this slice adds grouped fields to the log sheet, a `fieldset disabled` is how one would be grouped, and that is precisely the case the old selector could not express. It left one constraint on the rest of this slice, recorded in the Short-Term item above and repeated here because it binds step 4 specifically: **a dialog opened from inside the sheet is still not supported** — `inert` and `--sheet-h` are per-instance and unrefcounted, and a second Back closes both dialogs. The soreness card must not open a confirm from the sheet until that is fixed.
    2. **Pain and cadence** — the chips replacing the 0–10 number, then `intervalDays` on the profile with the expected date and days-late frozen onto each shot. Both are fully decided below and need no copy calls.
    3. **"Any days you felt off?"** — deferred out of the spine on purpose. Every open question about it was a guess at vocabulary for an interval whose length the app does not yet track, and step 2 is what makes that length real. The same reasoning the soreness card already uses for its buckets ("settle it when cadence lands rather than guessing now"). The four options are settled; what is not settled is whether they read right against a 3-day interval, and only shipping cadence answers that.
    4. **Soreness and bleeding** — the two remaining product surfaces, each with its own safety model, each large enough to deserve its own review.
