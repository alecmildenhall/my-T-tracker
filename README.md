...
- [ ] **History & Charts epic** (local-first, phone-first) — progressive-disclosure information architecture: Home shows a *recent-shots teaser* + "See all"; a dedicated **History** tab holds the full, filterable, searchable list **and** the charts; charts summarize trends. The unbounded list is never the main view. Built as thin slices, in order, after the CivilDate PR:
  ...
  **What we actually track (the signals the charts are built on).** Deliberately *not* pain-as-headline: pain is the easiest thing to log and the least interesting thing to chart. Industry pattern across the apps worth emulating (Daylio, Clue, Medisafe, Apple Health) is **fast ordinal scales + categorical tags**, with correlation — not raw numbers — as the payoff.
  ...
  - **How the site settled — soreness duration + an unabsorbed lump.** A different thing from the pain of the injection: an oil depot that hasn't absorbed leaves a palpable lump, and the site can be tender to touch for days afterwards. This is the half of the experience the current model has no room for, and it is the half worth showing a provider — "it was sore to touch for four days, mostly on the left" is a sentence you cannot currently produce from the data.

    **It is a *retrospective* field, and that is the whole design constraint.** The information does not exist when the shot is logged; you find out over the following week. So it is asked about the **previous** shot while logging the **next** one — the same slot, the same moment, and the same framing the ordinal mood already uses ("how the week since your last shot felt"). One "Since your last shot" block holds both, because they are the same kind of question and building them separately would mean designing that block twice. Always also editable from History, for anyone who prefers to record it the day it happens.

    Deliberately needs **no reminder or notification**: web notifications are unreliable on iOS, and a T tracker pinging a phone in public is an outing risk, so the design has to work without one.

    Stored as two optional, PII-free fields — `afterSoreness?: "none" | "day-or-two" | "several-days" | "week-plus"` and `afterLump?: boolean`. Four coarse buckets rather than a day count, on purpose: answering 7–14 days later is recall, and a number would claim a precision the memory does not have.

    **`undefined` (not answered) must never collapse into `"none"` (answered: it was fine).** If it does, every shot you skipped counts as painless and every chart tilts toward "no problem" — the same overloaded-value bug that twice re-dated a logged shot here. Some shots legitimately never get an answer (you skip a log; your most recent shot has no "next" yet), so **charts state their coverage** — "from 17 of 21 shots" — rather than quietly averaging over whatever they have.

    **Not opt-in, unlike bleeding**, and worth saying so explicitly because the reflex once one field is gated is to gate the next. Bleeding is off by default because showing it unasked risks dysphoria and cannot be undone; that reasoning is about the subject matter, not about caution generally. Soreness is a neutral fact about an injection, in the same register as pain and dose, and hiding it would mean most people never find the field that answers the question they actually have.

    **Charts it unlocks:** duration by **site** (the payoff, and directly actionable — it tells you where to rotate; extends the planned site-rotation view from injection-moment pain to lingering soreness), and duration by **carrier oil**, which the app already records and currently charts nothing with. Presented as plain counts with no interpretation, per the no-causal-claims rule below.
